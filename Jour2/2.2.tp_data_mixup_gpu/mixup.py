...
def mixup_data(x, y=None, num_classes=1000, alpha=1.):

    '''Compute the mixup data. Return mixed inputs, pairs of targets, and lambda'''
    batch_size = x.size()[0]
    
    # Get lambda values for each sample in batch
    _lambda = torch.tensor(np.random.beta(alpha, alpha, batch_size).astype('float32'))
    ...
    mixed_x = _lambda.view(batch_size, 1, 1, 1) * x + (1 - _lambda).view(batch_size, 1, 1, 1) * x[s_index, :]
